...
public class Diff {
	...
	static Logger debug = Logger.getRootLogger();
	...
	public boolean doDiff(String oldFile, String newFile) {
		...
		m_printOut.println( ">>>> Difference of file \"" + oldFile + 
			"\" and file \"" + newFile + "\".\n");
		try {
//			Node.traverseTree("Before first scan");
			m_oldInfo.inputScan(true);
//			Node.traverseTree("Before second scan");
			m_newInfo.inputScan(false);
//			Node.traverseTree("After second scan");
		} catch (IOException e) {
		...
		m_newInfo.initAlloc();
		transForm();
//		showBlocklen("after transform()");
//		m_oldInfo.showOther("old file");
//		m_newInfo.showOther("new file");
	...
	private void showBlocklen(String msg) {
		debug.info(">>> showBlocklen(); "+msg);
		for (int i=0; i<m_nBlocklen; i++) {
			debug.info("(i,value) ("+i+","+m_blocklen[i]+")");
		}
		debug.info("<<< showBlocklen()");
	}
