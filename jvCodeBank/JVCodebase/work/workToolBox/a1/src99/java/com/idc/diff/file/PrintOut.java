...
public class PrintOut {
	...
	private void showDelete() {
		if (m_printStatus != STATUS_DELETE)
			println( ">>>> DELETE AT " + m_printOldLine);
		m_printStatus = STATUS_DELETE;
		m_diff.getOldInfo().getSymbol(m_printOldLine).showSymbol();
		m_anyPrinted = true;
		m_printOldLine++;
	}
	...
	private void showInsert() {
		if (m_printStatus == STATUS_CHANGE)
			...
			println(">>>> INSERT BEFORE " + m_printOldLine);
		m_printStatus = STATUS_INSERT;
		m_diff.getNewInfo().getSymbol(m_printNewLine).showSymbol();
		m_anyPrinted = true;
		m_printNewLine++;
	}
	...
	private void showChange() {
		if (m_printStatus != STATUS_CHANGE) 
			println( ">>>> " + m_printOldLine + " CHANGED FROM");
