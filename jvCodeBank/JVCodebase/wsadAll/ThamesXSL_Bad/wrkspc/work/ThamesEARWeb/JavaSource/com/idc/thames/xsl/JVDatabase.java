...
public class JVDatabase {
	...
	public JVDatabase (String url, String user, String pwd) {
		Debug.println("url "+url+" user "+user+" password "+pwd);
		m_strURL = url;
		m_strUser = user;
		m_strPwd = pwd;
		m_conn = null;
	...
	public JVCatalog getCatalogItems (int nCatalogId) {
		...
		try {
			...
			while (rs.next()) {
				...
				int nPrice = rs.getInt(4);
				Debug.println("before addItem");
				catalog.addItem (new JVCatalogItem (nItemid,nCatalogId,strSds,strLds,nPrice));
				Debug.println("survived addItem");
			}
			Debug.println("before closeConnection()");
			closeConnection();
