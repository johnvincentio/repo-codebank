...
public class JVSendMessageSAAJ {
	...
	private static String m_strSendTo = "http://localhost:9083/Web1/ReceivingServlet";
//			"http://localhost:9083/DistributorWS2WAR/receiver";
	...
	public boolean createConnection() {
		...
		try {
			...
			System.out.println("Unable to open a SOAPConnection"+ex.getMessage());
			return false;
		}
		System.out.println("<<< createConnection");
		return true;
	...
	public void sendMessage (JVBarcodes barcodes) {
		...
		try {
			...
			while (iter.hasNext()) {
				...
				System.out.println("Barcodeid "+barcode.getBarcodeid());
			...
			System.out.println("m_strSendTo :"+m_strSendTo+":");
            URL urlEndpoint = new URL(m_strSendTo);
            System.out.println("Sending message to URLendPoint: "+urlEndpoint);

            SOAPMessage reply = m_connection.call(soapMessage, urlEndpoint);
            System.out.println("survived send message");

            if (reply != null)
            	System.out.println("there was a reply");
            else
