...
// addAlert; add an Alert - uses PL/SQL

	public void addAlert (int nEventid, int nRoleid, int nAlertlevel,
				int nAlertnumber, String strMsg) {
		Debug.println(">>> addAlert; index "+nEventid);
		try {
			makeConnection();
			StringBuffer buf = new StringBuffer();
			buf.append("begin ALERT_PKG.addAlertNext (?,?,?,?,?); end;");
			Debug.println("buf :"+buf+":");
			CallableStatement stmt = m_conn.prepareCall(buf.toString());
			Debug.println("survived prepareCall");
			stmt.setInt(1,nEventid);
			Debug.println("stage 21");
			stmt.setInt(2,nRoleid);
			Debug.println("stage 22");
			stmt.setInt(3,nAlertlevel);
			Debug.println("stage 23");
			stmt.setInt(4,nAlertnumber);
			Debug.println("stage 24");
			stmt.setString(5,strMsg);
			Debug.println("before execute");
			stmt.execute();
			...
			showSQLError("addAlert error",ex);
			m_bIsConnected = false;
		}
